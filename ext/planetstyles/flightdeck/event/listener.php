...
class listener implements EventSubscriberInterface
{
	...
	public function load_style_settings_data()
	{
		...
		$this->template->assign_vars(array(
			...
			'FORUM_LOGO_HEIGHT'		=> $this->get_forum_logo_height(),
	...
	protected function get_forum_logo()
	{
		...
		return $logo_src ? '<img class="custom_logo" src="' . generate_board_url() . '/' . $logo_src . '"' . $logo_width . $logo_height . ' alt="">' : '';
	}

	/**
	 * Get forum logo height
	 *
	 * @return string
	 */
	protected function get_forum_logo_height()
	{

		$logo_height = $this->config['style_settings_logo_height'] ? $this->config['style_settings_logo_height'] : '';

		return $logo_height ? $logo_height : '';
	}	
